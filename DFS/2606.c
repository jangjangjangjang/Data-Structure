...
AdjListNode* NewAdjListNode(int dest_computer) {
	...
	if (new_node == NULL) {
		perror("Failed to alocate memory for AdjListNode");
		exit(EXIT_FAILURE);
	}
...
Graph* CreateGraph(int num_computers) {
	...
	if (graph->adj_lists_array == NULL) {
		free(graph);
		perror("Failed to allocate memory for Adjist array");
		exit(EXIT_FAILURE);
	}
...
int main() {
	...
	if (color == NULL || pred == NULL || d == NULL || f == NULL) {
		perror("Failed to allocate memeory for DFS arrays");
